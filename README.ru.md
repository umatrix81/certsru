...
проверяются для разрешённых доменов и отклоняются везде остальном с ошибкой
...
Кросс-сертификат переиздаёт корень Минцифры под нашим собственным корнем,
...
| `myroot.pem` / `myroot.key` | Локальный корень. **Ключ — самое ценное.** |
...
```bash
rucerts ca list                           # корни, отпечатки ключей, статус кросс-сертификата
rucerts ca add new_root.cer               # добавить корень или распознать продление
...
`rucerts ca retire <имя>` переносит корень в `roots/retired/`, удаляет его
...
`install-certs.ps1` заново. Вывести из обращения единственный корень
инструмент откажется.

### Переименование локального корня
...
`backup-<метка времени>/`. Старый корень остаётся не у дел — удалите его из всех
...
в байтах, а не в символах — кириллица стоит два байта на символ.
...
списка между корнями), положительный контроль на живых сайтах, **отрицательный
контроль** и отпечатки исходных корней для обхода хранилищ доверия. Если хоть
одна проверка провалена, код возврата ненулевой.
...
```bash
rucerts init --cn "Мой ограничивающий корень"
...
только публичные сертификаты, но его установка делает ваш корень якорем доверия
...
**Одна неограниченная копия обнуляет всё.** Если исходный корень доверен
