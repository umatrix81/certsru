...
pub enum CaAction {
    ...
    /// Показать добавленные корневые сертификаты с отпечатками их ключей.
    List,
    /// Списать корневой сертификат, переместив его в roots/retired/.
    Retire {
...
#[derive(Debug, Subcommand)]
pub enum RootAction {
    /// Создать новый локальный корневой сертификат с указанным Common Name.
    ///
    /// Создаётся новая пара ключей; всё, что уже доверяет старому корневому
    /// сертификату, придётся обновить.
    SetCn {
        /// Новый Common Name.
        cn: String,
